...
grad_calc = lambda H, V0, φ, m: (abs(1.5*(H**2) - 0.5*V0*(φ**m)))**0.5
grad_sqr = lambda H, V0, φ, m: 1.5*(H**2) - 0.5*V0*(φ**m)
epsilon_calc = lambda  H, V0, φ, m: 2*(abs(1.5*(H**2) - 0.5*V0*(φ**m))/(H**2))
...
def trajectory(V_0, m, φ_0, dφ0 = 0.0001, boundmultiplier = 100, H0 = 2, suppress = False):
    ...
    while (epsilon < 1) and (n < boundmultiplier*T):
        ...
        #
        dφ = min(coeff*(H[n+1] - ((V_0*(φ[n+1])**m)*(1/3))**0.5)/(grad_calc(H[n+1], V_0, φ[n+1], m)), dφ0)
        n += 1
    return φ[:n+1], H[:n+1], n, epsilon #after the nth value of φ, H (if such exists) the values are the initial, meaningless zero values